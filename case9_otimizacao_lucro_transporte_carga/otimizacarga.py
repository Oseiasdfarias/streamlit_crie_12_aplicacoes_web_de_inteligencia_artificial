...
st.set_page_config(page_title="Otimização de Transporte de Carga",
                   layout="wide")
...
with col2.expander("Processamento"):
    if data is not None:
        sobra_peso = st.number_input("Informa a sobra de Peso",
                                     value=6000)
        sobra_volume = st.number_input("Informe a sobra de Volume",
                                       value=350)
        iteracao = st.number_input("Informe a quantidade de Iterações",
                                   value=10)
